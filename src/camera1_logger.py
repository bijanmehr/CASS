...
def set_action(msg):
    ...
    try:
        ...
        action = msg.data
        # action = action.split(',')
        with open(dir + '/camera1_logger.csv', 'a') as log_file :
            if action[1] == 'None':
                log_file.write('%d,%d,%s\n'%(current_frame,1,action))
                action = action[0]
            else:
...
def main():
    ...
    rospy.init_node('camera1_listener')
    ...
    rospy.Subscriber("/camera1/camera_info", CameraInfo, seq_callback)
